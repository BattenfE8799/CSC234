...
public:
    ...
    Room(int id, string name, string description, string exits) {
...
public:
    ...
    setup() {
        ...
        livingRoom.exits = "Exits: north south west";
        ...
        Room bedRoom = Room(BEDROOM, "Bedroom", "Simple bedroom with a comfy bed.", "Exits: south");
        bedRoom.south = LIVING_ROOM; //Bronze-changed connected room and direction
        roomList.at(BEDROOM) = bedRoom;

        Room kitchen = Room(KITCHEN, "kitchen", "A small, dimly lit kitchen.", "Exits: north east");
        ...
        Room backYard = Room(BACK_YARD, "Back Yard", "Roomy back yard with lots of plants.", "Exits: south");
        backYard.south = KITCHEN;
        roomList.at(BACK_YARD) = backYard;

        Room frontYard = Room(FRONT_YARD, "Front Yard", "Tidy front yard with lots of flowers.", "Exits: north");
